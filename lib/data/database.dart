...
class Database with Diagnosticable {
  ...
  Future<List<Town>> getTowns(County county) async =>
      (_data[county] ??= (await api.getTowns(county))
              .asMap()
              .map<Town, List<EdAction>>((int key, Town value) =>
  ...
  Future<void> loadAllTowns() async => api
      .getCounties()
      .forEach((County county) async => await getTowns(county));
